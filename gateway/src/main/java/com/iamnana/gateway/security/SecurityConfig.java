package com.iamnana.gateway.security;
...
//                        .pathMatchers("/orders/**").hasRole("ORDER")
    ...
    /**
     * This is a method that converts user roles from jwt. so it extract user roles from token.
     * @return
     */
    public Converter<Jwt, Mono<AbstractAuthenticationToken>> grantedAuthoritiesExtractor(){
        ...
        jwtAuthenticationConverter.setJwtGrantedAuthoritiesConverter(jwt -> {
            List<String> roles = jwt.getClaimAsMap("resource_access")
                    .entrySet()
                    .stream()
                    .filter(entry -> entry.getKey().equals("oauth-ecom-pkce"))
